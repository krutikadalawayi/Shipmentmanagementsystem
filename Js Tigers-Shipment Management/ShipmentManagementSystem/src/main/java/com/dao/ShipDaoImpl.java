...
public class ShipDaoImpl implements ShipDao{
	...
	@Override
	public void insertTodo(Ship todo) throws SQLException {
	...
	@Override
	public Ship selectTodo(long todoId) {
		Ship todo = null;
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			while (rs.next()) {
				...
				todo = new Ship(id, title, username, description, targetDate, isDone);
			}
		} catch (SQLException exception) {
			JDBCUtils.printSQLException(exception);
		}
	...
	@Override
	public List<Ship> selectAllTodos() {
		List<Ship> todos = new ArrayList<>();
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			while (rs.next()) {
				...
				LocalDate targetDate = rs.getDate("target_date").toLocalDate();
				boolean isDone = rs.getBoolean("is_done");
				todos.add(new Ship(id, title, username, description, targetDate, isDone));
			}
		} catch (SQLException exception) {
			JDBCUtils.printSQLException(exception);
		}
	...
	@Override
	public boolean updateTodo(Ship todo) throws SQLException {
		boolean rowUpdated;
		try (Connection connection = JDBCUtils.getConnection();
